...
const MeetingsTab = ({ meetings, onPlanClick, onResultClick, onCancelClick, onDeleteClick, onUpdateMeeting }) => {
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-4 space-y-4">
                ...
                <a
                    href="https://meetings.skrebeyko.ru/admin"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center justify-center w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 hover:text-slate-800 font-bold py-3.5 rounded-[2rem] transition-all shadow-sm hover:shadow-md gap-2"
                >
                    <CalendarIcon size={18} />
                    Внести встречи в расписание
                </a>
    ...
    const handleSavePlan = async () => {
        ...
        try {
            ...
            if (!meetingData.title || !meetingData.date) {
                alert('Пожалуйста, заполните название и дату встречи');
                setIsSaving(false);
                return;
            }
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex gap-2">
                                                        <select
                                                            ...
                                                            value={['Бесплатно', 'Free'].includes(formData.cost) ? 'free' : ['Донейшн', 'Donation'].includes(formData.cost) ? 'donation' : 'paid'}
                                                            onChange={(e) => {
                                                                const val = e.target.value;
                                                                if (val === 'free') setFormData({ ...formData, cost: 'Бесплатно' });
                                                                else if (val === 'donation') setFormData({ ...formData, cost: 'Донейшн' });
                                                                else setFormData({ ...formData, cost: '1000 RUB' });
                                                            }}
                                                        >
                                                        ...
                                                        {!['Бесплатно', 'Free', 'Донейшн', 'Donation'].includes(formData.cost) && (
                                                            <>
                                                                <input
                                                                    type="number"
                                                                    ...
                                                                    onChange={(e) => {
                                                                        const currency = formData.cost?.split(' ')[1] || 'RUB';
                                                                ...
                                                                <select
                                                                    ...
                                                                    value={formData.cost?.split(' ')[1] || 'RUB'}
                                                                    onChange={(e) => {
