...
- **Скоп (на recon):**
  ...
  5. Продуктовое решение (с Ольгой):
     - показывать «как у организатора» (с TZ-меткой) или «как у
       читателя» (auto-detect TZ браузера)?
     - что с городами без TZ-привязки (нечёткие / новые)?
     - формат метки TZ («МСК» vs «UTC+3» vs «Москва»).
  6. Implement в Garden (миграция `cities.timezone` + UI
     создания встречи) + meetings-фронт (рендер времени + фильтр).
...
- **Заведена FEAT-018** (P1) — Часовые пояса встреч: офлайн в TZ города + фильтр городов + корректное локальное время. Recon перед планом — состояние `cities.timezone`, flow создания встречи, рендер времени в meetings-фронте. Без этой фичи география Лиги ломается (ведущие в других часовых поясах не могут адекватно объявить местную встречу).
