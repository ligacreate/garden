---
title: FEAT-024 phase32 — STOP перед apply, нашёл утечку PII в §8 draft'а, нужен 🟢 на 2 новых дельты
date: 2026-05-16
from: VS Code Claude Code (codeexec)
to: стратег (claude.ai) + Ольга
reply_to: 2026-05-16_39_strategist_feat024_green.md + 2026-05-16_40_codeexec_feat024_apply_ready.md
type: diff-on-review BEFORE apply (НИЧЕГО не применено на проде, файл миграции НЕ создан)
status: 🛑 wait for green
---

# phase32 — pre-apply diff: убираю GRANT'ы → ensure_garden_grants трогать не надо

## 0. Почему stop, а не apply

Прошёл первый шаг чеклиста §3 файла `_40` — прочитал тело `ensure_garden_grants` на проде. По ходу — проверил кто owner таблиц и под кем пойдёт миграция. **Нашёл 2 проблемы в одобренном draft'е `_36 §6/§8`**, которые лучше починить ДО apply'я, а не после.

По проектному правилу diff-on-review (см. `feedback_diff_on_review.md` в auto-memory) — пишу diff, не амбициозничаю в обход.

## 1. Что показал prod-probe

```
DB_USER from .env: gen_user
current_user, session_user: gen_user, gen_user
rolname, rolsuper, rolcreaterole, rolcreatedb: gen_user|f|t|t
pg_tables.tableowner for profiles/pvl_student_homework_submissions/pvl_direct_messages: gen_user (все)
constraint profiles_access_status_check: ✅ существует (phase31 v3 closed)
public.tg_*: пусто
profiles.telegram*: только 'telegram' (FEAT-002 @username) — telegram_user_id/linked_at/notifications_enabled нет
```

Полное тело `ensure_garden_grants` — 6 частей, классический Tier-1/Tier-2/Tier-3 pattern, никаких TG-related grants. Длина 50+ строк. **phase31 v3 helper НЕ менял** — там никаких guard-grants нет.

## 2. Найденные проблемы в одобренном draft'е

### 🚨 Проблема 1 — GRANT'ы на TG-таблицы для `authenticated` = утечка PII
...
Логика была «authenticated должен читать для unlink/статуса», но на самом деле:
- **Unlink и статус привязки** живут в `profiles.telegram_*` (старый GRANT на `profiles` уже даёт доступ). Frontend ходит в `profiles` через PostgREST под JWT юзера.
- **`tg_link_codes`** должны генериться только через endpoint `/api/profile/generate-tg-link-code` в garden-auth (под `gen_user`). Если authenticated может SELECT'ить — может перебирать чужие активные `LINK-XXXXXX` и угонять привязки. Если может INSERT/UPDATE — может подделывать.
- **`tg_notifications_queue`** содержит `message_text` всех нотификаций, включая полный текст DM-сообщений ментора. SELECT через PostgREST = **любой авторизованный юзер читает чужие личные сообщения**. P0-уровня утечка, если не словить до apply.

**Worker (`setInterval` внутри garden-auth) подключается под `gen_user`** (owner) — ему GRANT'ы не нужны, он и так может всё. Webhook handler и endpoint'ы из Phase 2 — там же.

**Дельта Δ5:** убрать оба `GRANT … TO authenticated` из §8 draft'а. Без замены (никакие RLS-политики не нужны — таблицы не доступны authenticated в принципе).

### 🚨 Проблема 2 — `ensure_garden_grants` расширять не нужно

Стратег в `_39` писал:
> в одну транзакцию phase32 добавь `CREATE OR REPLACE FUNCTION public.ensure_garden_grants()` с расширенным телом (текущее тело + новые таблицы)

Это было правильно **при условии** что новые таблицы должны иметь GRANT'ы для authenticated (иначе Timeweb daily ACL-wipe их зачистит). Но раз GRANT'ов нет — нечего восстанавливать.

**Дельта Δ6:** **не трогать** `ensure_garden_grants` в phase32. Helper остаётся 1-в-1 как сейчас.

Побочный плюс: phase32 становится **строго additive** (новые объекты), без `CREATE OR REPLACE FUNCTION` на уже живой helper → нет риска нечаянно сломать существующий поведение helper'а.

## 3. Итоговый список всех дельт vs `_36 §6` (ВКЛЮЧАЯ старые)

| # | Источник | Содержание |
|---|---|---|
| Δ1 | `_39` 🟢 | `RAISE NOTICE 'tg-trigger: mentor unresolved for student_id=%'` перед `RETURN NEW` в ветке `to_status='submitted'` |
| Δ2 | `_39` 🟢 → **отменяется через Δ6** | ~~CREATE OR REPLACE FUNCTION ensure_garden_grants с расширенным телом~~ |
| Δ3 | `_39` 🟢 | «🔄 Просьба доработать ДЗ» вместо «🔄 Ментор просит доработать ДЗ» |
| Δ4 | `_39` 🟢 | Q7 «один TG → один профиль»: отказывать, реализация в Phase 2 webhook |
| **Δ5** | **этот файл, новая** | Убрать `GRANT … TO authenticated` на `tg_link_codes` и `tg_notifications_queue` из §8 — утечка PII |
| **Δ6** | **этот файл, новая** | `ensure_garden_grants` не трогать (следствие Δ5: нечего восстанавливать) |

## 4. Финальный SQL миграции (с применёнными Δ1, Δ3, Δ5, Δ6)

> Это полный текст, который я положу в `migrations/2026-05-16_phase32_tg_notifications.sql` **после** твоего 🟢. Δ2 и Δ4 — не в миграции (Δ2 отменена, Δ4 в Phase 2 Node-коде).
...
BEGIN
    ...
    IF NEW.to_status = 'submitted' THEN
        ...
        END IF;
        -- Δ1: если ментор не резолвится — оставляем NOTICE для видимости в логе.
    ...
    -- 5. Формируем текст. Δ3: «Просьба доработать ДЗ» (без «Ментор просит»).
...
## 5. Что осталось от чеклиста `_40 §3` после Δ5/Δ6

- ~~1. Прочитать тело `ensure_garden_grants`~~ — сделано (см. §1).
- ~~2. Смержить тело + новые GRANT'ы~~ — **отменено по Δ6** (не трогаем helper).
- 3. **Создать `migrations/2026-05-16_phase32_tg_notifications.sql`** из §4 этого файла. ← следующий шаг после 🟢
- 4. scp + `psql -v ON_ERROR_STOP=1 -f`
- 5. VERIFY V1-V12
- 6. Smoke (минимальный, без отправки в TG — бота нет ещё)
- 7. Commit миграции + `_session/` (по правилу проекта)
- 8. Отчёт `docs/_session/2026-05-16_42_codeexec_phase32_applied.md`

## 6. Что прошу

**🟢/🔴 на:**
- **Δ5** — убрать GRANT'ы authenticated на tg_link_codes + tg_notifications_queue (это утечка PII).
- **Δ6** — следствие Δ5: ensure_garden_grants не трогаем.
- финальный текст SQL из §4 этого файла.

Если 🟢 — иду по §5 без дополнительных уточнений.
Если 🔴 на Δ5 (хочешь оставить GRANT'ы под защитой RLS) — нужен отдельный заход на RLS-политики «only own rows», +0.3 сессии.

## 7. Что НЕ сделано

- ❌ Файл `migrations/2026-05-16_phase32_tg_notifications.sql` не создан.
- ❌ `psql -f` на прод не запущен. БД не тронута.
- ❌ Локальный `garden-auth/server.js` не правился.
- ❌ Никаких commit'ов.
- ✅ Сделан только readonly probe `pg_get_functiondef` + `pg_tables.tableowner` + `information_schema.columns` на проде.
