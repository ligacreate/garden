-- Поток 1 · завтраки: один прогон (вместо цепочки миграций 016–021).
-- — подтягивает строку Инны на 26.04, если её ещё нет (старые БД);
-- — синхронизирует title/description с data/pvl/seed.js;
-- — правит заголовки Романовой; убирает дубликат Кокориной 02.05.
-- Идемпотентно. Ручной прогон: этот файл; CI: migrations/022_pvl_calendar_flow1_breakfasts_sync.sql (тот же текст).

CREATE EXTENSION IF NOT EXISTS pgcrypto;

BEGIN;

-- Инна Кулиш 19.04: даты/время и текст (если строка есть)
...
-- Инна Кулиш 26.04: вставка, если отсутствует (014 на новых инсталляциях уже создаёт)
