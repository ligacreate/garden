...
import { Search, MapPin, Sparkles, Filter, X, Zap } from 'lucide-react';
...
const UserCard = ({ user }) => {
    // Simplify: Merge directions and skills into one visuals list
    const allTags = [...(user.directions || []), ...(user.skills || [])];
    ...
    const getTenure = () => {
        ...
        const remMonths = totalMonths % 12; // Keep for decimal if needed, or just years
        ...
        // Display as decimal for precision, but verify word match.
        // actually "2.5 года" reads okay. "5.1 лет" reads okay.
        // Let's stick to simple integer years if > 1 to avoid "2.1 лет" mess, or just standard grammar.
        // User asked specifically about "2 years".
        // Let's do: "2 года 3 мес" style or just properly declined years.

        // Simpler decimal approach with checking the *integer* part for the word, which is a common convention
        // "2.4 года" sounds better than "2.4 лет".

        return `${years}.${remMonths} ${yearWord} в Лиге`;
    };
            ...
            <div className="bg-white w-full max-w-lg rounded-[2rem] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] relative" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-8 pb-4 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="space-y-6">
                        ...
                        {/* Directions & Skills */}
                        {((user.directions && user.directions.length > 0) || (user.skills && user.skills.length > 0)) && (
                            <div className="space-y-6">
                                {user.directions && user.directions.length > 0 && (
                                    <div className="text-center">
                                        <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3">Направления</h4>
                                        <div className="flex flex-wrap gap-3 justify-center">
                                            {user.directions.map((tag, i) => (
                                ...
                                {user.skills && user.skills.length > 0 && (
                                    <div className="text-center">
                                        <h4 className="text-xs font-bold uppercase tracking-widest text-slate-400 mb-3">Навыки</h4>
                                        <div className="flex flex-wrap gap-3 justify-center">
...
const MapView = ({ users, currentUser, onSendRay }) => {
    ...
    const filteredUsers = useMemo(() => {
        return users.filter(user => {
            ...
            const matchDirection = !selectedDirection || (user.directions && user.directions.includes(selectedDirection));
            const matchSkill = !selectedSkill || (user.skills && Array.isArray(user.skills) && user.skills.includes(selectedSkill));
            ...
            return matchSearch && matchCity && matchDirection && matchSkill;
        });
    }, [users, search, selectedCity, selectedDirection, selectedSkill]);

    // Extract Options for Selects
    const cities = useMemo(() => [...new Set(users.map(u => u.city).filter(Boolean))].sort(), [users]);
    const directions = useMemo(() => [...new Set(users.flatMap(u => (Array.isArray(u.directions) ? u.directions : [])))].sort(), [users]);
    const skills = useMemo(() => [...new Set(users.flatMap(u => (Array.isArray(u.skills) ? u.skills : [])))].sort(), [users]);
    ...
    const hasActiveFilters = search || selectedCity || selectedDirection || selectedSkill;
        ...
        <div className="flex flex-col pt-6 animate-in fade-in slide-in-from-bottom-4 duration-500 px-4 lg:px-0">
            ...
            <div className="bg-white p-4 rounded-[2rem] border border-slate-200 shadow-sm mb-6 flex flex-col lg:flex-row gap-4">
                ...
                <div className="flex flex-wrap gap-2 flex-[3]">
                    ...
                    <FilterSelect icon={Filter} placeholder="Направления" value={selectedDirection} onChange={setSelectedDirection} options={directions} />
                    <FilterSelect icon={Sparkles} placeholder="Навыки" value={selectedSkill} onChange={setSelectedSkill} options={skills} />
                </div>
                <button
