        ...
        <div className="min-h-screen bg-transparent flex flex-col items-center justify-center p-6 relative overflow-hidden">
            <div className="w-full max-w-md relative z-10">
                <div className="relative min-h-[620px] overflow-hidden">
                    <div
                        className={`absolute inset-0 transition-opacity duration-400 ease-in-out ${step === 1 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                    >
                        <div className="surface-card p-8 space-y-4">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h2 className="text-2xl font-display font-semibold text-slate-900">Регистрация</h2>
                            </div>
                            <div className="space-y-3">
                                <Input placeholder="Имя и фамилия" value={regData.name} onChange={e => setRegData({ ...regData, name: e.target.value })} />
                                <Input placeholder="Email" value={regData.email} onChange={e => setRegData({ ...regData, email: e.target.value })} />
                                <Input type="password" placeholder="Пароль" value={regData.password} onChange={e => setRegData({ ...regData, password: e.target.value })} />
                                <Input label="Введите дату рождения" type="date" value={regData.dob} max={new Date().toISOString().split("T")[0]} onChange={e => setRegData({ ...regData, dob: e.target.value })} />
                                <p className="text-[10px] text-slate-400">Нужна для определения вашего дерева.</p>
                            </div>
                            <Button onClick={handleRegisterCalculate} className="w-full mt-4" disabled={!regData.email}>{isProcessing ? "Магия..." : "Далее"}</Button>
                        </div>
                    </div>

                    <div
                        className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${step === 2 ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
                    >
