...
2. *Срок хранения истекает* — получатель, телефон, состав, трек. Триггер:
   до `keep_free_until` из заказа осталось меньше `CDEK_EXPIRY_WARN_HOURS`
   (двое суток), а посылку не забрали.

   Срок берём у СДЭК, а не считаем сами: у разных пунктов он разный — по живым
   заказам издательства встречались и 7 дней, и 14. Свой таймер такого не
   угадает и позовёт либо рано, либо к шапочному разбору. `CDEK_WAITING_HOURS`
   остался запасным: срабатывает, только когда СДЭК срока не дал.
...
**Почему не ждём письма СДЭК и не ловим статус.** Письмо про истечение срока
хранения приходит Ольге на почту — его пришлось бы читать через IMAP. А статус
заказа СДЭК меняет только когда посылка уже поехала назад: это поздно.
`keep_free_until` в самом заказе даёт тот же срок, что и письмо, без доступа
к почте — сверено с отчётом СДЭК от 05.08 на четырёх посылках, совпало.
