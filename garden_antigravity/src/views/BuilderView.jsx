...
const SaveScenarioModal = ({ onSave, checkActionTimer, onClose, user }) => {
        ...
        <div className="fixed inset-0 z-[60] bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white w-full max-w-sm rounded-3xl flex flex-col shadow-2xl p-6 animate-in zoom-in-95 duration-200">
                ...
                {canPublish ? (
                    ...
                    <div className="flex items-center gap-3 mb-6 p-2 opacity-50 cursor-not-allowed">
                        ...
                        <div className="flex-1">
                            ...
                            <div className="text-xs text-slate-400">Доступно для стажеров и ведущих</div>
                        </div>
                    </div>
                )}
...
const BuilderView = ({ practices, timeline, setTimeline, onNotify, user, onSave }) => {
    ...
    return (
        <div className="h-full flex flex-col pt-6 px-4 lg:px-0">
            ...
            {showSaveModal && <SaveScenarioModal onSave={handleSave} onClose={() => setShowSaveModal(false)} user={user} />}
