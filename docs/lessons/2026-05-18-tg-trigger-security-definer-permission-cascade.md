...
✅ **Natural acceptance — 2026-05-19 11:06 МСК.** Ольга Разжигаева сдала
свою первую ДЗ. Запись в `pvl_homework_status_history` создалась, trigger
`tg_enqueue_homework_event` выстрелил под SECURITY DEFINER, INSERT в
`tg_notifications_queue` прошёл, worker garden-auth (на long-polling с
sessии _72) подхватил, **Василина получила push `hw_submitted_new` в
TG через ~5 секунд** после клика «Отправить».

Phase36 = `82b0a6c` (apply + миграция), lesson опубликован в
`82381c6`. Закрыто `BUG-HW-SUBMIT-NO-HISTORY` + `BUG-PVL-FRONTEND-STUDENT-HISTORY-WRITE`
(второй — `26b5c54`, parallel-баг на frontend стороне с early-return
ДО loop'а status-history; см. отдельный lesson
[`2026-05-19-pvl-first-submit-early-return.md`](2026-05-19-pvl-first-submit-early-return.md)).

> **Подтверждено end-to-end:** frontend submission → DB INSERT →
> trigger SECURITY DEFINER → queue INSERT → worker poll → TG-push
> ментору. Гипотеза permission-cascade — корневая, fix корректный.
