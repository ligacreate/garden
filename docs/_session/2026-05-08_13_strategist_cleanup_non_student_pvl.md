# Cleanup non-student pvl_students records

**Адресат:** VS Code Claude Code (executor).
**Автор:** стратег (claude.ai).
**Создано:** 2026-05-08.

🟢 на data-cleanup миграцию.

## Контекст

Из 22 записей `pvl_students` 5 — не настоящие студентки (попали через
системный баг `ensurePvlStudentInDb` без проверки роли):

| UUID | ФИО | Profile role | hw | content | mentor_links_as_mentor |
|---|---|---|---:|---:|---:|
| `e6de2a97-60f8-4864-a6d9-eb7da2831bf4` | Анастасия Зобнина | admin | 0 | 2 | 0 |
| `4250ffac-acd7-4209-bd28-b31bd9c02665` | Анастасия Ван | intern | 0 | 5 | 0 |
| `6cf385c3-5d4b-44dc-bcf1-6aa17d50bac7` | Василина Лузина | **mentor** (для 4 студенток!) | 0 | 2 | 4 |
| `628585ef-a6c2-4e1b-b4c6-bf49b5ecc839` | Наталья Гулякова | mentor | 0 | 1 | 0 |
| `33333333-3333-3333-3333-333333333301` | Участница | NULL (тест-фикстура) | 7 | 23 | 0 |

## Задача

Создать `migrations/data/2026-05-08_cleanup_non_student_pvl_records.sql`:
...
--
--   - 1 admin (Анастасия Зобнина — твой ассистент Настя)
--   - 1 intern (Анастасия Ван — стажёр)
--   - 2 mentor (Василина Лузина, Наталья Гулькова)
...
После apply через ssh+psql:
- `git add migrations/data/2026-05-08_cleanup_non_student_pvl_records.sql`
- `git commit -m "data: cleanup pvl_students от 5 не-студенческих записей"`
  (Co-Authored-By строка обязательна)
- `git push`

В commit message — упомянуть, что architectural fix
`BUG-PVL-ENSURE-RESPECTS-ROLE` отдельным тикетом.

## После apply

Стратег верифицирует:
1. `count(*) FROM pvl_students = 17`
2. Через FEAT-017 (Cmd+Shift+R) — таблица должна показать 17 строк.
3. Подопечные Василины (4 студентки) продолжают показывать её имя в
   столбце «Ментор» через profiles.name COALESCE.

## Что НЕ делаем

- Удаление `profiles` / `users_auth` для этих 5 — нет, это реальные
  пользователи (кроме «Участницы»).
- Architectural fix `ensurePvlStudentInDb` — отдельный тикет.

## Жду от executor'a

Apply-отчёт в файле:
```
docs/_session/2026-05-08_14_codeexec_cleanup_apply_report.md
```

Со снимком до/после, commit hash, push результат.
