# BUG-TG-TRIGGER-STATUS-MISMATCH — diff (v2, узкий — вариант A)

**От:** codeexec (VS Code)
**Кому:** Ольга → стратегу
**Дата:** 2026-05-18
**В ответ на:** [_61](2026-05-18_61_strategist_bug_tg_trigger_status_mismatch.md) + Ольгин recon-correction в чате
**Статус:** 🟢 от Ольги (вариант A, доверие на apply). После compact-diff → apply на прод.

---

## Recon-correction

Первая версия _62 ошибочно заявила «триггеры отсутствуют». Причина — фильтр `WHERE tgname LIKE 'tg_%'`, который не ловит фактические имена `trg_tg_*`. Перепроверка:

```sql
SELECT tgname, tgrelid::regclass, tgenabled
  FROM pg_trigger
 WHERE tgrelid IN ('pvl_homework_status_history'::regclass,
                   'pvl_direct_messages'::regclass)
   AND NOT tgisinternal;

trg_tg_enqueue_homework_event       | pvl_homework_status_history | O
trg_tg_enqueue_direct_message_event | pvl_direct_messages         | O
```

Оба триггера ENABLED (`O` = origin). Очередь пуста потому что до 17.05 ни одна студентка не привязала Telegram — функция упиралась в `IF v_recipient_tg IS NULL THEN RETURN NEW`. Ирина Петруня — первая привязанная (17.05 15:06).

**→ Никакого extra-scope. Минимальный fix: одна строка в теле функции.**

---

## Миграция (вариант A, ~60 строк)
...
Effective code-diff с текущей функцией:
...
Всё остальное — обёртка миграции (pre/post assertions + ensure_garden_grants + транзакция).
