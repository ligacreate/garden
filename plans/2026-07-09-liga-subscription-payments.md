...
- [~] 1d — UI «Моя подписка»: **backend return_url задеплоен** (`?paid=1`); **фронт (ProfileView карточка + App/UserApp возврат-хендлер) закоммичен, ждёт push** (одно окно 403 вместе с 1e)
- [~] 1e — ручная отметка: **backend задеплоен + smoke ✓** (миграция phase46 + mark-paid endpoint); **фронт (AdminPanel) закоммичен, ждёт push** (CI→FTP = окно 403, в спокойное время)
- [ ] 1f — напоминания
