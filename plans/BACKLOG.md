...
### DESIGN-001: Объединение дизайн-системы (две палитры → одна) + доступность
- **Приоритет:** P1 (фаза доступности — выше по человеческому импакту)
- **Блокер:** нет
...
- **ADMIN-MOBILE-RESPONSIVE (P2):** pre-existing адаптивная задолженность AdminPanel, найдена при B4-верификации (Chrome 390px/563px), НЕ регресс B4 — B4 эти места не трогал. (1) Таб-ряд [AdminPanel.jsx:760](../views/AdminPanel.jsx) `flex gap-1 w-fit` без `overflow-x-auto`/`flex-wrap` → 8 вкладок переполняют экран, лишние клипаются (body `overflow-x:hidden`), недостижимы тачем. Воспроизводится и на 390px. Фикс: `overflow-x-auto flex-nowrap` (+ `shrink-0` на табах) или wrap. (2) Таблица пользователей [AdminPanel.jsx:1188](../views/AdminPanel.jsx) `surface-card p-8 overflow-hidden` + `w-full` таблица → колонка «Действия» клипается. Фикс: обёртка `overflow-x-auto` вместо `overflow-hidden` (горизонтальный скролл таблицы) или card-раскладка строк на мобиле. (3) Низкий приоритет: fixed mobile-хедеры (`md:hidden fixed w-full`, [UserApp.jsx:875](../views/UserApp.jsx),1097) на viewport 480–768px смещены на `(W−480)/2` из-за центрирующей колонки `max-w-[480px]` ([App.jsx:568](../App.jsx)); на реальных телефонах ≤480px (целевые 360–430px) колонка = весь экран → дефект НЕ виден. Чинить только если нужна полоса 480–768px. AdminPanel — admin-инструмент Ольги, мобильная адаптивность не в приоритете → P2.
...
### FEAT-026: Сертификационный завтрак — двойная самооценка (менти + ментор) + сравнение
- **Статус:** 🟢 DONE 2026-05-31 (на проде, ЗАКРЫТ тумблером — ждёт ручного открытия приёма)
- **Приоритет:** P1
- **Создано:** 2026-05-28
- **Закрыто:** 2026-05-31 — фронт собран, e2e на тест-паре зелёный, задеплоен закрытым.
- **Что это:** менти и её ментор независимо (parallel-blind) оценивают проведённый
  сертификационный завтрак по одной анкете — 18 критериев (1–3) + 6 рефлексий;
  критические флаги отмечает ментор. После того как обе стороны отправили —
  открывается экран сравнения (баллы рядом, расхождения ≥2, рефлексии бок-о-бок,
  рекомендация ментора). Платформа вердикт НЕ выносит — решение принимают живые
  люди (ментор+менти, в сложном — Ирина). Admin может вернуть сторону на пересдачу.
- **Что в проде:**
  - БД `phase40` (28.05): таблицы `pvl_student_certification_self` / `_mentor`,
    RLS parallel-blind (чужую оценку видно только после её submit), авто-`mentor_id` триггером.
  - Фронт (Сессии 2–4, коммит `fb12e8f`): API-слой, `PvlCertificationBlock`
    (switcher self/mentor/compare/locked), wizard на реальном API с autosave,
    `PvlCertificationCompareView`, admin-пересдача. Развязаны страницы:
    «Сертификация» = правила, «Моя страница» = бланк. Поправлен общий логин
    (`RemoteApiService` пишет `garden_currentUser` — закрыт баг `u-st-1`).
  - **Тумблер приёма по когорте** `phase42` (`pvl_cohorts.certification_open`,
    default false) + admin-переключатель в `AdminPvlProgress` (Garden AdminPanel).
    Закрыто → LockedCard «приём откроется позже»; открыто → бланк активен; admin всегда видит превью.
  - Реестр Потока 1 `phase43` (коммит `f4d76d3`): дашборд и cert-таргетинг
    показывают только текущих абитуриенток (`role='applicant'`), выпускницы-`intern`
    исключены (29 → 16).
- **Открытые followup'ы:**
  - **Открыть приём** тумблером (`/admin/pvl`, Поток 1) — в день старта, ~через 2 недели (Ольга).
  - (опц.) почистить тест-записи феи/фиксика в `_self`/`_mentor` перед открытием.
  - Решить: справочник правил полный (6 разделов) vs урезать (см. `_171`).
  - **CERT-RLS-DEFENSE-IN-DEPTH** (P3) — RLS на INSERT cert по роли (доп. защита, вне Этапа 2).
- **Связано:** ТЗ `docs/_session/2026-05-28_144_strategist_tz_etap2_certification.md`,
  рефлексии `_146`, тумблер-ТЗ `_171`, отчёты `_165…_175`. Тест-пара: Настина фея / фиксик, Поток 1.
...
### MEETING-ADMIN-DELETE-ANY: дать админу удалять ЛЮБУЮ встречу (не только свою)
- **Статус:** 🔴 TODO
- **Приоритет:** P3 (не горит — закрытое доверенное сообщество; потребность Маши = self-delete ведущей, она уже работает)
- **Создано:** 2026-06-25 (побочка фичи безопасного удаления, сессия `_205`)
- **Контекст:** в проде на `meetings` есть только RLS-политика `"Users can delete own meetings."`
  (USING `auth.uid() = user_id` + гард status/seeds, phase45). Админской delete-any-политики
  нет, и UI-удаления встреч у админа тоже нет (кнопка живёт только в кабинете ведущей,
  `MeetingCard.jsx`). Миграция 08 определяла `meetings_delete_admin`, но в проде её нет.
- **Задача:** при необходимости — RLS-политика `meetings_delete_admin` (USING `public.is_admin()`)
  + точка входа в админке. Учесть тот же гард по последствиям (или осознанно разрешить
  админу удалять completed — тогда нужен и откат семян, см. ниже).
- **Связано:** фича `_205`, [[MEETING-COHOST-SEED-REFUND]], migration phase45.

### MEETING-COHOST-SEED-REFUND: откат +5 со-ведущим при удалении встречи
- **Статус:** 🔴 TODO
- **Приоритет:** P3 (мелкая утечка семян; редкий кейс — у тестовой встречи обычно нет со-ведущих)
- **Создано:** 2026-06-25 (вынесено из фичи `_205` — намеренно НЕ угадывал математику RPC)
- **Контекст:** при удалении no-consequence встречи хосту откатывается +5 за создание
  (`UserApp.handleDeleteMeeting`, клампится в 0 на своём профиле). Со-ведущим +5 за создание
  начислялся через `increment_user_seeds(co_hosts, 5)` (SECURITY DEFINER RPC). Симметричный
  откат `(-5)` НЕ сделан: RPC — `set seeds = coalesce(seeds,0) + amount` — **не клампит** и
  применяет одну сумму ко всем сразу → у со-ведущего с балансом < 5 уведёт в минус. А
  напрямую PATCH'ить чужой профиль ведущая не может (RLS profiles = own-only), поэтому
  клампить per-user через текущий RPC нельзя.
- **Задача:** либо новый/правленый RPC с per-user клампом (`greatest(0, seeds - amount)`),
  либо отдельная definer-функция отката, вызываемая на удалении. Тогда включить откат
  со-ведущим в `handleDeleteMeeting`.
- **Связано:** фича `_205`, `increment_user_seeds`, [[MEETING-ADMIN-DELETE-ANY]].

### INFRA-005-ASSETS-RETENTION: чистка `assets/` на проде от чанков старше ~30д
- **Статус:** 🔴 TODO
- **Приоритет:** P3 (не горит — файлы мелкие JS/CSS; растёт медленно)
- **Создано:** 2026-06-09 (побочка корневого фикса INFRA-005, commit `0736b75`)
- **Контекст:** корневой фикс chunk-flap (deploy.yml: префетч live `assets/` +
  мерж перед clean-slate-выкладкой) делает деплой **additive** — старые
  хеш-чанки больше не удаляются, чтобы долгие вкладки догрузили свои модули.
  Цена: `assets/` на сервере **растёт каждый деплой** (каждая выкладка качает
  все старые + заливает все старые+новые). См. урок
  [`docs/lessons/2026-06-09-infra005-chunk-flap.md`](../docs/lessons/2026-06-09-infra005-chunk-flap.md).
- **Задача:** периодическая чистка `assets/` — удалять чанки старше ~30д
  (или оставлять N последних сборок). Механизм на выбор: отдельный lftp-шаг в
  deploy.yml (`mirror`/`glob rm` по mtime), либо cron на сервере.
- **Осторожно:** не удалять чанки, на которые ссылается текущий `index.html`,
  и оставлять запас под живые вкладки (≥30д разумно). Контролируемый рост вместо
  мгновенного 404 — нынешнее состояние уже безопасно, тикет — гигиена.
- **Связано:** [[INFRA-005-SW-CACHE]] (симптомный фикс через ErrorBoundary),
  урок `2026-06-09-infra005-chunk-flap.md`, сессии `_182/_184/_185/_186`.

### BUG-CALENDAR-EVENT-TYPE-DRIFT: normalizeCalendarEventTypeForDb маппит типы по-разному в mock и postgrest
- **Статус:** 🔴 TODO
- **Приоритет:** P3 (понижено с P2 2026-06-01 после recon `_177`: ПРОД КОРРЕКТЕН — CHECK-constraint + UI-маппинг; чинить только mock-дрейф, не горит)
...
- **Вердикт recon** (`docs/_session/2026-05-31_177_codeexec_recon_calendar_event_type_drift.md`): прод-путь (postgrest) пишет ТОЛЬКО CHECK-валидные канонические типы — констрейнт `pvl_calendar_events_event_type_check` (миграция `010`: `lesson/practicum/practicum_done/breakfast/mentor_meeting/live_stream/lesson_release/deadline/other`), а UI (`PvlCalendarBlock.jsx`: лейблы, цвет точки, навигация, архив-фильтр) их читает и рендерит корректно. Расхождение влияет ТОЛЬКО на dev-mock (в реальную БД не пишет). Влияние на пользователя нулевое — канонический тип и его legacy-аналог дают тот же лейбл и цвет.
- **Проблема:** функция существует в обоих API-слоях, маппит 4 типа по-разному:
  - [`services/pvlPostgrestApi.js:147`](../services/pvlPostgrestApi.js#L147) (prod): `mentor_meeting→mentor_meeting`, `lesson_release→lesson_release`, `live_stream→live_stream`, `session→mentor_meeting` — все CHECK-валидны.
  - [`services/pvlMockApi.js:526`](../services/pvlMockApi.js#L526) (mock, `// legacy types:`): `mentor_meeting→practicum`, `lesson_release→lesson`, `live_stream→breakfast`, `session→practicum`.
- **Что сделать (не горит):** привести MOCK-маппинг к прод-варианту (postgrest = источник правды = схема + UI), затем функцию можно дедуплицировать в `services/pvlTransforms.js`. **Прод-маппинг НЕ менять.**
- **Acceptance:** mock даёт те же значения, что postgrest; либо явно задокументировано, почему legacy-сворачивание в mock намеренно.
- **Связано:** commit `d804e60` (care-refactoring R1), recon `docs/_session/2026-05-31_177_codeexec_recon_calendar_event_type_drift.md`, `services/pvlMockApi.js:526`, `services/pvlPostgrestApi.js:147`.
